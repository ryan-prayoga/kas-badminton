...
import { SCORE_FORMATS, suggestFee, TOURNAMENT_SIZES } from "@/lib/domain/tournament";
import type { KokType, PlayerRow, ScoreFormat, TournamentSize } from "@/lib/domain/types";
...
export function CreateTournamentSheet({
  ...
  const [lines, setLines] = useState<KokLine[]>(() => [newKokLine(active, defaultPrice, "0")]);
  ...
  const kokCount = totalKokFromLines(lines);
  const kokTotal = totalRupiahFromLines(lines, active, defaultPrice);
  const feeHint = suggestFee(kokTotal, participants);

  const changeSize = (next: TournamentSize) => {
    setSize(next);
    // Pertahankan pasangan yang sudah diketik saat ganti ukuran bagan.
    setPairs((prev) => emptyPairs(next).map((empty, i) => prev[i] ?? empty));
  };
  ...
  const reset = () => {
    ...
    setFee("");
    setNotes("");
    setLines([newKokLine(active, defaultPrice, "0")]);
  };
  ...
  const submit = () => {
    ...
    const filled = pairs.flatMap((p) => [p.a, p.b]).map((n) => n.trim().toLowerCase()).filter(Boolean);
    ...
    start(async () => {
      const res = await safeAction(() =>
        createTournamentAction({
          ...
          koks: kokLinesToKoks(lines, active, defaultPrice),
  ...
  return (
    <Sheet
      ...
      onOpenChange={(next) => {
        setOpen(next);
        if (next) setLines([newKokLine(active, defaultPrice, "0")]);
      }}
    >
      <SheetTrigger
        render={
      ...
      >
        <SheetHeader className="shrink-0 pr-12">
          ...
          <SheetDescription>
            Bagan gugur. Slot yang dikosongin otomatis jadi BYE.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 pb-4">
          ...
          {multiDay ? (
            <div className="space-y-1.5">
              ...
              {/* Turnamen bisa masih berjalan — tanggal depan boleh, tapi tak boleh sebelum mulai. */}
              <DateField
                id="t-end-date"
                value={endDate}
                onChange={setEndDate}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-[11px] text-ink-faint">{participants} peserta</span>
            </div>
            {pairs.map((p, i) => (
          ...
          <p className="-mt-2 text-[11px] text-ink-faint">
            Kok turnamen memotong stok, sama seperti catat main. Boleh 0 dulu, bisa ditambah
            belakangan.
          </p>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] text-ink-faint">
              {kokCount > 0 && participants > 0 ? (
                <>
                  {kokCount} kok ≈ {fmt(kokTotal)} · saran{" "}
                  <button
                    type="button"
                    onClick={() => setFee(String(feeHint))}
                    className="font-semibold text-court underline underline-offset-2"
                  >
                    {fmt(feeHint)}/orang
                  </button>
                </>
              ) : (
                "Isi 0 kalau turnamen ini gratis — tidak akan muncul di rekap tagihan."
              )}
            </p>
          </div>
